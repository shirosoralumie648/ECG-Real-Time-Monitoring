...
class ECGDataProcessor:
    ...
    def clean_signal(self, ecg_signal, sampling_rate=280):
        """
        低通滤波器，只保留 80Hz 以下的信号
        ...
        try:
            ...
            # 设计一个第6阶的Butterworth低通滤波器，截止频率为100Hz
            cutoff = 100  # Hz
            nyquist = sampling_rate / 2  # 奇奇斯特频率
            normal_cutoff = cutoff / nyquist  # 归一化截止频率
            
            # 设计滤波器
            b, a = signal.butter(6, normal_cutoff, btype='low', analog=False)
            
            # 应用滤波器，使用前向-后向滤波，避免相位偏移
            filtered_signal = signal.filtfilt(b, a, ecg_signal)
            
            # 应用移动平均进行额外平滑以提高连续性
            window_size = max(3, int(sampling_rate / 120))  # 通过采样率计算平滑窗口
            filtered_signal = np.convolve(filtered_signal, np.ones(window_size)/window_size, mode='same')
            
            # 保留基线偏移去除，但仅在必要时使用
            median_val = np.median(filtered_signal)
            filtered_signal = filtered_signal - median_val
            ...
            print(f"80Hz低通滤波失败: {e}")
            # 如果滤波失败，尝试使用简单的移动平均滤波
            try:
                # 使用移动平均进行简单的低通滤波
                window_size = max(3, int(sampling_rate / 80))  # 根据80Hz频率计算窗口大小
                filtered_signal = np.convolve(ecg_signal, np.ones(window_size)/window_size, mode='same')
